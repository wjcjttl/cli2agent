...
一个自托管的 Docker 服务，封装 AI 编程 CLI（Claude Code、Codex、Gemini、OpenCode、Kimi）并将其暴露为 HTTP + SSE + MCP 端点。即使供应商访问中断，也能保持你的 AI 工具持续运行。
...
**在访问中断时保持 AI 工作流持续运行。** 近期各大模型供应商的封号潮让许多团队措手不及 — Slack 机器人沉默了，CI/CD 流水线断了，多智能体编排系统一夜之间失去了核心引擎。如果你依赖 Claude Code 来驱动 OpenClaw、Cline 或自定义集成，一次账户限制就能让所有下游工具全部瘫痪。

cli2agent 是你的工具与 AI 编程智能体之间的弹性中间层：

- **工具访问不中断** — OpenClaw、Slack 机器人、CI/CD 流水线和编排系统调用 cli2agent 的 API。如果认证路径变更，只需更新一个环境变量 — 所有下游集成继续正常运行。
- **多条认证路径** — API 密钥、OAuth（Claude Pro/Max）、AWS Bedrock、Google Vertex AI。如果一条路径被限制，切换到另一条，无需改动集成中的任何代码。
- **多后端故障转移** — 不仅限于 Claude。将 `CLI2AGENT_CLI_BACKEND` 切换为 `codex`、`gemini`、`opencode` 或 `kimi`，让工作流在完全不同的供应商上继续运行。
- **完全自托管，完全可控** — 你的基础设施、你的凭据、你的正常运行时间。不依赖任何供应商的 Web UI、桌面应用或平台可用性。
- **一个 API 统一一切** — 将任何 AI 编程 CLI 暴露为 HTTP + SSE + MCP 端点。只需对接 cli2agent 一次，后端随意切换。
...
最简单的方式，适用于任何 Anthropic API 套餐。与 OAuth 是独立的认证路径 — 在消费者计划中断时可作为备选方案。
...
> **注意：** OAuth 令牌与你的消费者计划绑定。如果计划受限，此方式将无法使用。cli2agent 支持多种认证方式 — 你可以切换到 API 密钥（方式一）或 Bedrock/Vertex（方式四/五），无需更改任何下游集成。
...
通过 AWS Bedrock 使用 Claude。由你的 AWS 协议管理的独立认证路径 — 适合需要保证可用性的团队，或在其他认证方式中断时作为备选方案。
...
通过 Google Cloud 使用 Claude。与 Bedrock 类似，由你的 GCP 协议管理的独立认证路径。
