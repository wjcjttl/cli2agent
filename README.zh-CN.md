...
一个自托管的 Docker 服务，封装 AI 编程 CLI（Claude Code、Codex、Gemini、OpenCode、Kimi）并将其暴露为 HTTP + SSE API 端点。使用 API 密钥无头运行 Claude Code — 无需订阅计划。
...
**你的 API 密钥在订阅计划失效时依然可用。** 消费者计划（Claude Pro、Max）可能会受到账户限制和访问变更的影响，一夜之间打断你的工作流。API 密钥、AWS Bedrock 和 Google Vertex AI 独立计费、独立运作 — 你的访问权限取决于 API 协议，而非消费者订阅。

cli2agent 让你能够：

- **使用 API 密钥继续使用 Claude Code** — 无需 Pro/Max 订阅。只要你有 `ANTHROPIC_API_KEY`，就有访问权限。
- **通过企业通道接入** — AWS Bedrock 和 Google Vertex AI 提供 SLA 保障，不受消费者计划变更影响。只需切换一个环境变量。
- **即时切换供应商** — 如果 Claude 不可用，将 `CLI2AGENT_CLI_BACKEND` 改为 `codex`、`gemini`、`opencode` 或 `kimi`，继续工作。
- **完全自托管** — 你的代码、你的凭据、你的基础设施。不依赖任何供应商的 Web UI 或桌面应用。
- **随处集成** — 将 Claude Code 暴露为 API，供 Cline、Cursor、LangChain、CI/CD 流水线、Slack 机器人或任何 HTTP 客户端使用。
...
最简单、最可靠的方式。API 密钥按 token 计费，独立于任何消费者订阅计划运作。你的 API 访问不受 Pro/Max 计划状态的影响。
...
> **注意：** OAuth 令牌与你的消费者计划绑定。如果你的计划被暂停或限制，此方式将无法使用。建议切换到方式一（API 密钥）或方式四/五（Bedrock/Vertex）以获得不间断的访问。
...
通过 AWS Bedrock 使用 Claude。Bedrock 访问由你的 AWS 协议管理，独立于 Anthropic 消费者计划 — 非常适合需要保证可用性的团队。
...
通过 Google Cloud 使用 Claude。与 Bedrock 类似，Vertex AI 访问由你的 GCP 协议管理，独立于 Anthropic 消费者计划运作。
